...
export function OverviewView({ allGoals, stats, trophies: _trophies, streak }: OverviewViewProps) {
  ...
  const statusCounts = statusOrder.map((s) => ({
    s,
    n: allGoals.filter((g) => g.status === s).length,
  }));
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-4 px-5 py-4 bg-[linear-gradient(135deg,oklch(0.22_0.06_82_/_0.5),oklch(0.18_0.05_270_/_0.5))] border border-[oklch(0.74_0.17_85_/_0.2)] rounded-[var(--r)]">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[11px] text-[var(--text-hi)] italic leading-[1.5]">
            &ldquo;{motiv.quote}&rdquo;
          </p>
          <p
              ...
              {statusCounts.map(({ s, n }) =>
                n > 0 ? (
                  <div
                    key={s}
                    className={cn(
                      'flex items-center justify-center text-[8px] font-bold text-[var(--panel)] rounded-[3px] transition-[flex] duration-500',
                      STATUS_BAR_STYLE[s],
                    )}
                    style={{ flex: n }}
                  >
          ...
          <div className="px-4 py-3 flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
            <div className="relative text-[var(--gold)] shrink-0">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-[8px] text-[var(--text-lo)] tracking-[0.08em] uppercase mt-0.5">
                  Complete
                </span>
              </div>
            </div>
            <div className="flex flex-col gap-3 w-full sm:flex-1">
              {[
                ...
                <div key={label} className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold text-[var(--text-hi)] font-[var(--font-title)]">
                    {val}
                  </span>
                </div>
              ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className={panel}>
          <div className="px-4 py-3 flex flex-col gap-3">
            <Icon icon="🔥" className="text-[28px] leading-none text-[var(--gold)]" />
            <div>
              <span className="font-[var(--font-title)] text-[28px] font-black text-[var(--gold)] leading-none">
                {streak}
              </span>
              <span className="text-[13px] text-[var(--text-lo)] ml-1.5">days</span>
            </div>
            <div>
              <div className="text-[11px] font-bold text-[var(--text-hi)] font-[var(--font-title)]">
                Current Streak
              </div>
            ...
            <div className="flex gap-1 pt-1">
              {weekDots.map(({ d, lit, today }, i) => (
                <div key={i} className="flex flex-col items-center gap-0.5 flex-1">
                  ...
                  <span
                    className={cn(
                      'text-[8px] font-[var(--font-title)] font-bold',
                      today ? 'text-[var(--gold)]' : 'text-[var(--text-lo)]',
                    )}
                  >
                    {d}
                  </span>
        ...
        <div className={panel}>
          <div className="px-4 py-3 flex flex-col gap-3">
            ...
            <div>
              <span className="font-[var(--font-title)] text-[28px] font-black text-[var(--violet)] leading-none">
                {stats.milestonesDone}
              </span>
              <span className="text-[13px] text-[var(--text-lo)] ml-0.5">
            ...
            <div>
              ...
              <div className="text-[9px] text-[var(--text-lo)] mt-0.5">
                {Math.round((stats.milestonesDone / (stats.milestonesTotal || 1)) * 100)}% of all
                steps complete
              </div>
            </div>
          ...
          <div className="px-4 py-3 flex flex-col gap-2.5">
            {catDist.map((c) => (
              <div key={c.id} className="flex items-center gap-2">
                ...
                <span className="text-[9px] text-[var(--text-mid)] w-[52px] shrink-0 font-[var(--font-title)] tracking-[0.04em]">
                  {c.label}
                </span>
                ...
                <span className="text-[10px] font-bold text-[var(--text-hi)] w-4 text-right font-[var(--font-title)]">
                  {c.n}
                </span>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className={panel}>
          ...
          <div className="px-4 py-3 flex flex-col gap-4 overflow-y-auto max-h-[280px]">
            {months.length === 0 ? (
              <p className="text-[10px] text-[var(--text-lo)] py-4 text-center">
                No goals to display
              </p>
            ) : (
              months.map((m) => {
        ...
        <div className={panel}>
          ...
          <div className="px-4 py-3 flex flex-col gap-2">
            {upcoming.length === 0 ? (
              <p className="text-[10px] text-[var(--text-lo)] py-6 text-center">
              ...
              upcoming.map((g) => {
                ...
                return (
                  <div
                    key={g.id}
                    className="flex items-center gap-3 px-3 py-2.5 bg-[var(--panel2)] rounded-[var(--r-sm)]"
                  >
                    <div className="flex flex-col items-center shrink-0 w-10 text-center">
                      <span
                        className="font-[var(--font-title)] text-[18px] font-black leading-none"
                        style={{ color: catAccent }}
                      >
                        {g.daysLeft}
                      </span>
                      <span className="text-[8px] text-[var(--text-lo)] uppercase tracking-[0.06em]">
                        days
                      </span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-[11px] font-semibold text-[var(--text-hi)] truncate">
                        {g.title}
                      </div>
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <div className="flex-1 h-[3px] bg-[var(--panel)] rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: `${g.progress * 100}%`, background: catAccent }}
                          />
                        </div>
                        <span className="text-[9px] text-[var(--text-lo)]">
  ...
  return (
    <div className="flex items-center gap-2 px-4 pt-3 pb-2.5 border-b border-[var(--border)]">
      <div className="flex flex-col gap-0.5 flex-1">
        <span className="text-[8px] text-[var(--gold)] tracking-[0.12em] uppercase font-[var(--font-title)]">
          {eyebrow}
        </span>
        <span className="text-[11px] font-bold text-[var(--text-hi)] font-[var(--font-title)] tracking-[0.04em]">
          {title}
        </span>
      </div>
...
const panel = 'bg-[var(--panel)] border border-[var(--border)] rounded-[var(--r)] overflow-hidden';
