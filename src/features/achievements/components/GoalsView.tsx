...
const STS_ORDER: Record<string, number> = {
  'in-progress': 0,
  'not-started': 1,
  completed: 2,
  archived: 3,
...
export function GoalsView({ goals, stats, onAction, onToggleMilestone, onNew }: GoalsViewProps) {
  ...
  const visible = useMemo(() => {
    ...
    list = [...list].sort((a, b) => {
      if (sortBy === 'priority')
        return (
          (PRI_ORDER[a.priority] ?? 9) - (PRI_ORDER[b.priority] ?? 9) || b.progress - a.progress
        );
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Stat strip — hidden on mobile to keep the goals list front and center */}
      <div className="hidden sm:grid sm:grid-cols-3 lg:grid-cols-5 gap-2 lg:gap-3">
        <StatCard
          tone="gold"
          ico="◎"
          label="Total Goals"
          value={stats.total}
          sub={`${stats.active} in motion`}
        />
        ...
        <StatCard
          ...
          sub={`${stats.completedThisYear} this year`}
        />
        <StatCard
          tone="violet"
          ico="✧"
          ...
          sub={`${stats.lockedTrophies} within reach`}
        />
        <StatCard
          tone="gold"
          ico="◐"
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {CATEGORIES.map((c) => (
            <button
              ...
              className={cn(
                chipBase,
                filterCat === c.id ? chipActive : '',
                c.textClass,
                filterCat !== c.id && 'opacity-60',
              )}
            >
              {c.ci} {c.label}
            </button>
      ...
      {visible.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-4 py-20 text-center">
          ...
          <p className="text-[11px] text-[var(--text-lo)] max-w-[320px] leading-relaxed">
            Every legend starts with a single declared intent. Forge your first ambition and the
            journey begins.
          </p>
          <button
            type="button"
