...
const CAT_COLOR: Record<GoalCategory, string> = {
  career: 'var(--gold)', health: 'var(--mint)', learning: 'var(--cyan)',
  finance: 'var(--violet)', personal: 'var(--rose)',
};
const CAT_GLOW: Record<GoalCategory, string> = {
  career: 'var(--gold-glow)', health: 'var(--mint-glow)', learning: 'var(--cyan-glow)',
  finance: 'var(--violet-glow)', personal: 'var(--rose-glow)',
};
...
interface Particle {
  ...
  life: number; decay: number; wob: number; wobs: number;
}
...
function buildBoard(goals: Goal[]): (string | null)[] {
  ...
  active.forEach((g, i) => { board[i] = g.id; });
  return board;
}

function loadNotes(): AllNotes {
  try { return JSON.parse(localStorage.getItem(LS_NOTES) ?? '{}') as AllNotes; } catch { return {}; }
}
...
function HudTile({ label, ico, accent, glow, value, suffix, of: ofVal, sub }: {
  label: string; ico: string; accent: string; glow: string; value: number;
  suffix?: string; of?: number; sub: string;
}) {
  ...
  return (
    ...
    >
      ...
      <div className="font-[var(--font-title)] text-[22px] font-extrabold leading-none flex items-baseline gap-1 text-[var(--text-hi)]">
        ...
        {suffix && <small className="text-[11px] text-[var(--text-lo)] font-semibold">{suffix}</small>}
        {ofVal != null && <span className="text-[12px] text-[var(--text-dim)] font-bold">/{ofVal}</span>}
      </div>
      <div className="font-[var(--font-code)] text-[9px] text-[var(--text-lo)] mt-1.5 tracking-[0.04em]">{sub}</div>
    </div>
  );
}
...
function BingoCellItem({
  goal, idx, dealt, justDone, lineWin, onOpen,
}: {
  goal: Goal | null; idx: number; dealt: boolean;
  justDone: boolean; lineWin: boolean; onOpen: (idx: number) => void;
}) {
  ...
  const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (el) {
      ...
      el.appendChild(rip);
      requestAnimationFrame(() => { rip.style.transform = 'translate(-50%,-50%) scale(1)'; rip.style.opacity = '0'; });
    ...
    return (
      <div
        ...
        style={{
          background: 'linear-gradient(180deg,var(--surface-2),var(--surface))',
          aspectRatio: '1',
          animation: dealt ? `bingo-deal 0.6s cubic-bezier(0.34,1.56,0.64,1) ${delay}ms both` : undefined,
  ...
  const priColor = { high: 'var(--rose)', medium: 'var(--gold)', low: 'var(--cyan)' }[goal.priority];
    ...
    >
      ...
      {done && (
        <div
          ...
          style={{
            background: 'linear-gradient(120deg, transparent 38%, oklch(1 0 0 / 0.14) 50%, transparent 62%)',
      ...
      <div className={cn('flex items-center gap-1.5 mb-auto', done && 'opacity-55')}>
        ...
        <span
          className={cn('w-[18px] h-[18px] rounded-[4px] inline-flex items-center justify-center font-[var(--font-title)] text-[9px] font-black shrink-0', RANK_STYLE[goal.rank])}
        >
          {goal.rank}
        </span>
        <span className="ml-auto text-[8px] leading-none tracking-[-0.5px]" style={{ color: priColor }}>
          {priToken}
        </span>
      </div>
      ...
      {!done && (
        <div className="flex items-center gap-1.5 mt-auto">
          ...
          <span className="font-[var(--font-code)] text-[9px] min-w-[26px] text-right" style={{ color: accent }}>
            {pct}%
          </span>
        </div>
      )}
        ...
        <>
          <div
            ...
            style={{
              ...
              color: 'var(--gold)',
              background: 'radial-gradient(circle, var(--gold-glow), oklch(0.78 0.16 82 / 0.05) 70%)',
              border: '1.5px solid oklch(0.78 0.16 82 / 0.55)',
              boxShadow: '0 0 18px var(--gold-glow)',
              textShadow: '0 0 10px var(--gold-glow)',
              animation: justDone ? 'bingo-daub 0.55s cubic-bezier(0.34,1.56,0.64,1) both' : undefined,
            }}
          >
            {goal.linkedTrophy ? '♛' : '✓'}
          </div>
...
function BingoBoardGrid({
  cells, dealt, justDoneId, lineWinCells, onOpen,
}: {
  cells: (Goal | null)[];
  dealt: boolean;
          ...
          return (
            ...
            >
                  ...
                  <span
                    ...
                    style={
                      cells[ci]?.status === 'completed'
                        ? { background: 'var(--gold)', borderColor: 'var(--gold)', boxShadow: '0 0 6px var(--gold-glow)' }
              ...
              {won ? (
                ...
                <span className="font-[var(--font-code)] text-[9px] text-[var(--text-dim)]">{done}/4</span>
              )}
            </div>
          );
        })}
...
function BingoLegend() {
  const items = [
    { chip: '🔒', chipClass: 'opacity-60 saturate-[0.4] text-[var(--text-lo)]', name: 'Locked',      desc: 'Not yet begun — tap to open' },
    { chip: '◐',  chipClass: 'text-[var(--cyan)] border-[oklch(0.78_0.16_205_/_0.5)] shadow-[0_0_10px_var(--cyan-glow)]', name: 'In Progress', desc: 'Underway, pulsing with intent' },
    { chip: '✓',  chipClass: 'text-[var(--gold)] border-[var(--gold)] shadow-[0_0_10px_var(--gold-glow)]', name: 'Conquered', desc: 'Daubed — counts toward a line' },
  ];
  return (
    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[var(--r-lg)] p-3.5">
        ...
        {items.map(it => (
          <div key={it.name} className="flex items-center gap-2.5 py-1.5">
            ...
            <div className="min-w-0">
              <div className="font-[var(--font-title)] text-[11px] tracking-[0.04em] text-[var(--text-hi)]">{it.name}</div>
...
function BingoDetailPanel({
  goal, notes, onClose, onToggleMilestone, onComplete, onAddNote, onEditNote, onDeleteNote,
}: {
  goal: Goal;
  notes: Note[];
  onClose: () => void;
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-2 mb-2.5">
            <span
              ...
              style={{ color: accent, borderColor: accent, background: `oklch(from ${accent} l c h / 0.1)` }}
            >
              {cat.ci} {cat.label}
            </span>
            <span className={cn('w-[22px] h-[22px] rounded-[5px] inline-flex items-center justify-center font-[var(--font-title)] text-[11px] font-black', RANK_STYLE[goal.rank])}>
              {goal.rank}
            </span>
            {!done && (
              <span className={cn('ml-auto font-[var(--font-title)] text-[8px] tracking-[0.14em] font-bold px-2.5 py-[3px] rounded-full border uppercase', priCls)}>
                {priLabel}
              </span>
            )}
          </div>
          ...
          <div className="flex gap-4 mt-3.5 items-center">
            <div className="relative w-[60px] h-[60px] shrink-0" style={{ color: accent }}>
              ...
              <div className="absolute inset-0 flex items-center justify-center font-[var(--font-title)] text-[16px] font-extrabold" style={{ color: accent }}>
                {done ? '✓' : pct}
              </div>
            </div>
            <div className="flex gap-2">
              ...
              <span className="inline-flex items-baseline gap-1 font-[var(--font-title)] text-[14px] font-extrabold px-3 py-[5px] rounded-full bg-[var(--bg-2)] border text-[var(--gold)] border-[oklch(0.78_0.16_82_/_0.45)]">
                {goal.coins} <small className="text-[8px] tracking-[0.14em] opacity-70">COINS</small>
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2.5">
              {[
                { k: 'Status',    v: goal.status === 'completed' ? 'Completed' : goal.status === 'in-progress' ? 'In Progress' : 'Not Started' },
                { k: 'Rank',      v: goal.rank },
                ...
                <div key={k} className="bg-[var(--bg-2)] border border-[var(--border)] rounded-[var(--r-sm)] px-2.5 py-2">
                  <div className="font-[var(--font-title)] text-[7.5px] tracking-[0.16em] text-[var(--text-dim)] uppercase mb-1">{k}</div>
          ...
          {goal.milestones.length > 0 && (
            <div>
              <div className="font-[var(--font-title)] text-[9px] tracking-[0.22em] text-[var(--text-lo)] uppercase mb-2 flex items-center gap-2">
                ◈ &nbsp; Milestones
                <span className="ml-auto font-[var(--font-code)] text-[var(--text-dim)]">{msDone}/{goal.milestones.length}</span>
              </div>
                ...
                {goal.milestones.map(m => (
                  ...
                  >
                    <span
                      ...
                      style={m.done ? { background: accent, boxShadow: `0 0 8px ${accent}` } : undefined}
                    >
                      {m.done ? '✓' : ''}
                    </span>
                    <span className={cn('text-[12px]', m.done ? 'text-[var(--text-dim)] line-through' : 'text-[var(--text-md)]')}>
                      {m.label}
                    </span>
                  </div>
                ))}
          ...
          {goal.note && (
            ...
            >
              <div className="font-[var(--font-title)] text-[8px] tracking-[0.24em] mb-1" style={{ color: accent }}>◆ STRATEGY</div>
              <div className="text-[11.5px] text-[var(--text-md)] leading-[1.55] italic">{goal.note}</div>
            </div>
          )}
          ...
          {goal.linkedTrophy && (
            <div className="font-[var(--font-code)] text-[10px] text-[var(--text-lo)]">
              ✧ Completing this unlocks the <span className="text-[var(--gold)]">{goal.linkedTrophy}</span> trophy
            </div>
          )}
          ...
          <div>
            <div className="font-[var(--font-title)] text-[9px] tracking-[0.22em] text-[var(--text-lo)] uppercase mb-2 flex items-center gap-2">
              ✎ &nbsp; Chronicle · Notes
              <span className="ml-auto font-[var(--font-code)] text-[var(--text-dim)]">{notes.length}</span>
            </div>
            ...
            <div className="flex flex-col gap-2 mb-3">
              ...
              <div className="flex items-center gap-2">
                <span className="font-[var(--font-code)] text-[9px] text-[var(--text-dim)]">⌘ + ⏎ to save</span>
                <button
                  type="button"
                  disabled={!draft.trim()}
                  onClick={addNote}
                  ...
                  style={noteSaved ? undefined : { background: 'linear-gradient(135deg,var(--gold-2),var(--gold))', boxShadow: '0 0 12px var(--gold-glow)' }}
                >
              ...
              <div className="flex flex-col gap-2.5">
                {notes.map(note => (
                  <NoteCard
                    ...
                    editingId={editingId}
                    editText={editText}
                    onStartEdit={() => { setEditingId(note.id); setEditText(note.text); }}
        ...
        <div className="shrink-0 px-5 py-3.5 border-t border-[var(--border)] bg-[var(--bg-2)] flex gap-2.5">
          {done ? (
            <div className="flex-1 flex items-center justify-center gap-2 rounded-[9px] py-3 font-[var(--font-title)] text-[12px] font-extrabold tracking-[0.14em] uppercase text-[#021] shadow-[0_0_18px_var(--mint-glow)]"
...
function NoteCard({
  note, accent, editingId, editText,
  onStartEdit, onCancelEdit, onSaveEdit, onEditTextChange, onDelete,
}: {
  ...
  return (
    ...
    >
      {editing ? (
        ...
        <>
          <div className="text-[12px] text-[var(--text-md)] leading-[1.55] whitespace-pre-wrap break-words">{note.text}</div>
          <div className="flex items-center gap-2.5 mt-2 group">
            <span className="font-[var(--font-code)] text-[9px] text-[var(--text-dim)] tracking-[0.04em]">
              ◷ {note.ts}{note.edited && <span className="italic"> · edited</span>}
            </span>
...
function RewardPopup({ data, onDone }: { data: RewardData; onDone: () => void }) {
  ...
  return (
    <div
      ...
      role="status"
    >
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse at top, var(--gold-glow), transparent 60%)', opacity: 0.5 }} />
      <div
      ...
      <div className="relative font-[var(--font-title)] text-[9px] tracking-[0.3em] text-[var(--gold)] mb-1.5">✦ &nbsp; AMBITION CONQUERED &nbsp; ✦</div>
      <div className="relative font-[var(--font-title)] text-[22px] font-extrabold tracking-[0.04em] leading-[1.1] text-[var(--text-hi)]">{data.title}</div>
      <div className="relative text-[12px] text-[var(--text-md)] mt-1.5">{cat.label} · Rank {data.rank}</div>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ width: 10, height: 10, boxShadow: '0 0 40px var(--gold-glow)', animation: 'bingo-wave 1.1s cubic-bezier(0.16,1,0.3,1) forwards' }}
      />
      <div
        className="font-[var(--font-title)] font-black tracking-[0.14em] leading-none"
        style={{
          fontSize: 'clamp(64px,16vw,200px)',
          background: 'linear-gradient(135deg,var(--gold) 0%,oklch(0.85 0.2 60) 30%,var(--violet) 70%,var(--cyan) 100%)',
      ...
      <div
        ...
        style={{ fontSize: 'clamp(13px,2vw,19px)', textShadow: '0 2px 14px oklch(0 0 0 / 0.8)', animation: 'fade-in 0.6s 0.2s ease both' }}
      >
        {data.cheer}
      </div>
      <div
...
function AchievementToast({ data, onDone }: { data: AchData; onDone: () => void }) {
  ...
  return (
    <div
      ...
      role="status"
    >
      <div className="w-11 h-11 rounded-full shrink-0 flex items-center justify-center text-[22px] text-[var(--gold)] border-2 border-[var(--gold)]"
        style={{ background: 'radial-gradient(circle, var(--gold-glow), transparent 70%)', boxShadow: '0 0 16px var(--gold-glow)' }}>
        ♛
      </div>
      <div className="min-w-0 flex-1">
        <div className="font-[var(--font-title)] text-[8px] tracking-[0.24em] text-[var(--gold)] mb-0.5">✧ &nbsp; TROPHY UNLOCKED</div>
        <div className="font-[var(--font-title)] text-[15px] font-bold text-[var(--text-hi)] tracking-[0.02em]">{data.name}</div>
        <div className="font-[var(--font-code)] text-[10px] text-[var(--violet)] mt-0.5">{data.reward}</div>
      </div>
    </div>
  );
}
...
export function BingoView({ goals, streak, coins, gems, onCompleteGoal, onToggleMilestone }: BingoViewProps) {
  /* ── board state ── */
  const [boardGoalIds, setBoardGoalIds] = useState<(string | null)[]>(() => loadBoard() ?? Array(16).fill(null));
  const boardInitialized = useRef(false);

  // Initialize or reset board from goals
  useEffect(() => {
    if (boardInitialized.current) return;
    if (goals.length === 0) return;
    const saved = loadBoard();
    if (saved) {
      // validate: keep IDs that still exist (even if now archived for display)
      const knownIds = new Set(goals.map(g => g.id));
      const validated = saved.map(id => (id && knownIds.has(id) ? id : null));
      setBoardGoalIds(validated);
    } else {
  ...
  useEffect(() => {
    if (!boardInitialized.current) return;
    try { localStorage.setItem(LS_BOARD, JSON.stringify(boardGoalIds)); } catch {}
  }, [boardGoalIds]);

  /* ── notes ── */
  const [allNotes, setAllNotes] = useState<AllNotes>(loadNotes);
  useEffect(() => {
    try { localStorage.setItem(LS_NOTES, JSON.stringify(allNotes)); } catch {}
  }, [allNotes]);
  ...
  useEffect(() => {
    ...
    const spawn = (x: number, y: number, opts: Partial<{ count: number; spread: number; angle: number; power: number; gravity: number; colors: string[]; upward: boolean }> = {}) => {
      const { count = 120, spread = Math.PI * 2, angle = -Math.PI / 2, power = 9, gravity = 0.18, colors = FX_COLORS, upward = false } = opts;
    ...
    const loop = () => {
      ...
      for (let i = particlesRef.current.length - 1; i >= 0; i--) {
        ...
        p.vy += p.g; p.vx *= 0.99; p.wob += p.wobs;
        p.x += p.vx + Math.cos(p.wob) * 0.6; p.y += p.vy; p.rot += p.vrot; p.life -= p.decay;
        if (p.life <= 0 || p.y > window.innerHeight + 40) { particlesRef.current.splice(i, 1); continue; }
        ctx.save();
  ...
  const stats = useMemo(() => {
    ...
    const xpEarned  = boardCells.filter(c => c?.status === 'completed').reduce((a, c) => a + (c?.xp ?? 0), 0);
  ...
  useEffect(() => {
    ...
    if (prevCompletedIds.current === null) { prevCompletedIds.current = cur; return; }
    ...
    if (newIds.length > 0) {
      ...
      if (goal) {
        ...
        setReward({ title: goal.title, cat: goal.cat, rank: goal.rank, xp: goal.xp, coins: goal.coins });
        if (goal.linkedTrophy) {
          setTimeout(() => setAch({ name: goal.linkedTrophy!, reward: `+${goal.xp} XP · +${goal.coins} ◉` }), 650);
        }
      }
    }
  ...
  useEffect(() => {
    if (prevWonLineIds.current === null) { prevWonLineIds.current = new Set(wonLines.map(l => l.id)); return; }
    ...
    if (newLines.length > 0) {
      ...
      setTimeout(() => {
        ...
        shake();
        setBingo({ line: newLines[0]!.label, cheer: CHEERS[Math.floor(Math.random() * CHEERS.length)]! });
      }, 950);
    }
  ...
  const resetBoard = useCallback(() => {
    ...
    setSelectedIdx(null);
    setLineWinCells([]);
    setReward(null); setBingo(null); setAch(null);
    // Re-trigger entrance
    setDealt(false);
  ...
  const addNote    = useCallback((note: Note) => {
    ...
    setAllNotes(prev => ({ ...prev, [selectedGoal.id]: [note, ...(prev[selectedGoal.id] ?? [])] }));
  }, [selectedGoal]);
  const editNote   = useCallback((noteId: string, text: string) => {
    if (!selectedGoal) return;
    setAllNotes(prev => ({
      ...prev,
      [selectedGoal.id]: (prev[selectedGoal.id] ?? []).map(n => n.id === noteId ? { ...n, text, edited: true } : n),
    }));
  }, [selectedGoal]);
  const deleteNote = useCallback((noteId: string) => {
    if (!selectedGoal) return;
    setAllNotes(prev => ({ ...prev, [selectedGoal.id]: (prev[selectedGoal.id] ?? []).filter(n => n.id !== noteId) }));
  }, [selectedGoal]);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-end gap-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10.5px] text-[var(--text-mid)] leading-[1.55] max-w-[500px]">
            ...
            <span className="text-[var(--cyan)] font-bold"> row, column, or diagonal</span> to land a BINGO.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="grid grid-cols-4 sm:grid-cols-7 gap-2.5">
        ...
        <HudTile label="Streak"      ico="✦" accent="var(--rose)"   glow="var(--rose-glow)"   value={streak}          suffix="d" sub="unbroken" />
        <HudTile label="XP Earned"   ico="◆" accent="var(--violet)" glow="var(--violet-glow)" value={stats.xpEarned}  sub="from conquests" />
        <HudTile label="Bingo Lines" ico="⊞" accent="var(--gold)"   glow="var(--gold-glow)"   value={wonLineIds.size} of={10} sub="lines lit" />
        <HudTile label="Active"      ico="❖" accent="var(--cyan)"   glow="var(--cyan-glow)"   value={stats.active}    sub="in progress" />
      </div>
