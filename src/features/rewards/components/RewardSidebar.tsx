          ...
          {RARITIES.map((r) => {
            ...
            return (
              <div key={r.id} className="flex items-center gap-2">
                <span
                  className="text-[8px] font-[var(--font-title)] tracking-[0.1em]"
                  style={{ color: r.color, width: 52 }}
                >
...
const sidebarBase =
  'w-[85vw] max-w-[280px] lg:w-[196px] lg:max-w-none h-full shrink-0 flex flex-col gap-0 border-r border-[var(--border)] bg-[var(--panel)] overflow-y-auto';
const section = 'px-3 py-3 border-b border-[var(--border)]';
const sectionHead =
  'flex items-center gap-2 text-[9px] font-bold tracking-[0.18em] uppercase text-[var(--text-lo)] font-[var(--font-title)] mb-2';
const countBadge = 'font-[var(--font-mono)] text-[var(--text-dim)] font-normal';
const countNum = 'text-[9px] font-[var(--font-mono)] text-[var(--text-dim)]';
const glyph = 'text-[11px] text-[var(--text-lo)] w-4 text-center shrink-0';
...
const libRowActive = '!bg-[oklch(0.74_0.17_85_/_0.08)] !text-[var(--gold)] font-semibold';
...
const rarRowActive = '!bg-[var(--panel2)] !text-[var(--text-hi)] font-semibold';
const rarPip =
  'shrink-0 text-[8px] font-bold tracking-[0.08em] px-1 py-0.5 rounded border font-[var(--font-title)]';

const statRow =
  'w-full flex items-center gap-2 px-1.5 py-[5px] rounded-[var(--r-sm)] text-[11px] text-[var(--text-mid)] transition-colors hover:bg-[var(--panel2)] cursor-pointer';
const statRowActive = '!bg-[var(--panel2)] !text-[var(--text-hi)] font-semibold';
const statPip = 'w-2 h-2 rounded-full shrink-0';
