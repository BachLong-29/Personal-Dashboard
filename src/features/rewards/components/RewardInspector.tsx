...
import { RARITIES, STATUSES, REWARD_COLORS, COLOR_VALUES, RARITY_META, STATUS_META } from '../constants';
...
export function RewardInspector({
  reward, draft, setDraft, mode, setMode, dirty, publishing, onPublish, onDiscard,
    ...
    return (
      <aside className={inspBase}>
        <div className="flex flex-col items-center justify-center flex-1 gap-3 text-[var(--text-lo)]">
          ...
          <div className="font-[var(--font-title)] text-[11px] tracking-[0.14em]">SELECT A REWARD</div>
        </div>
      </aside>
    );
  }
  ...
  return (
    <aside className={inspBase} style={{ ['--rar-color' as string]: rar.color, ['--rar-glow' as string]: rar.glow }}>
      {/* Header */}
      <div className={inspHead}>
        <div className="flex flex-col gap-0.5">
          ...
          <div className="text-[9px] text-[var(--text-dim)] font-[var(--font-mono)] truncate">{draft.id}</div>
        </div>
        <div className={modeToggle}>
          <button type="button" className={cn(modeBtn, mode === 'edit' && modeBtnActive)} onClick={() => setMode('edit')}>
            Edit
          </button>
          <button type="button" className={cn(modeBtn, mode === 'preview' && modeBtnActive)} onClick={() => setMode('preview')}>
            Preview
          </button>
        </div>
      </div>
      ...
      <div
        ...
        style={{ borderColor: `${rar.color}55`, background: `radial-gradient(ellipse 120% 80% at 50% 0%, ${rar.bg} 0%, transparent 70%)` }}
      >
      ...
      <div className="flex-1 overflow-y-auto">
        {mode === 'edit' ? (
          <div className={form}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {RARITIES.map((r) => (
                  <button
                    ...
                    className={cn(
                      rarPickBtn,
                      draft.rarity === r.id && 'border-[var(--rar-pick-c)] !text-[var(--rar-pick-c)] bg-[var(--rar-pick-bg)]',
                    )}
                    style={{
              ...
              <div className="flex flex-wrap gap-1.5">
                {STATUSES.map((s) => (
                  <button
                    ...
                    className={cn(
                      statPickBtn,
                      draft.status === s.id && 'border-[var(--stat-c)] bg-[oklch(from_var(--stat-c)_l_c_h_/_0.1)]',
                    )}
                ...
                {REWARD_COLORS.map((c) => (
                  <button
                    ...
                    className={cn(
                      ...
                      draft.color === c ? 'scale-110 border-white shadow-[0_0_8px_var(--c)]' : 'border-transparent',
                    )}
                    style={{
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className={costInput}>
                  ...
                  <input
                    ...
                    onChange={(e) => setF('coinCost', e.target.value === '' ? null : Number(e.target.value))}
                    placeholder="—"
                  />
                </div>
                <div className={costInput}>
                  ...
                  <input
                    ...
                    min={0}
                    value={draft.gemCost ?? ''}
                    onChange={(e) => setF('gemCost', e.target.value === '' ? null : Number(e.target.value))}
                    placeholder="—"
                  />
                </div>
              </div>
            ...
            <Field label="Stock" hint="blank = unlimited">
              <input
                ...
                min={0}
                value={draft.stock ?? ''}
                onChange={(e) => setF('stock', e.target.value === '' ? null : Number(e.target.value))}
                placeholder="∞"
              />
            </Field>
          </div>
          ...
          <div className={form}>
            <PreviewRow label="Rarity"  value={RARITY_META[draft.rarity].label}       color={RARITY_META[draft.rarity].color} />
            <PreviewRow label="Status"  value={STATUS_META[draft.status].label}       color={STATUS_META[draft.status].color} />
            <PreviewRow label="Coins"   value={draft.coinCost != null ? `● ${draft.coinCost}` : '—'} color="var(--gold)" />
            <PreviewRow label="Gems"    value={draft.gemCost  != null ? `◆ ${draft.gemCost}`  : '—'} color="var(--violet)" />
            ...
            {draft.description && (
              <div className="mt-2 px-3 py-2 bg-[var(--panel)] border border-[var(--border)] rounded-[var(--r-sm)]">
                ...
                <p className="text-[11px] text-[var(--text-mid)] leading-[1.5] mt-1">{draft.description}</p>
              </div>
            )}
          </div>
        )}
  ...
  return (
    <div className="mb-3">
      <div className="flex items-center gap-1.5 mb-1.5">
        ...
        {hint && (
          <span className="ml-auto text-[9px] text-[var(--text-dim)] font-[var(--font-mono)]">{hint}</span>
        )}
      </div>
      {children}
    </div>
...
function PreviewRow({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="flex items-center justify-between px-3 py-1.5 border-b border-[var(--border)] last:border-0">
      <span className="text-[9px] font-bold tracking-[0.12em] uppercase text-[var(--text-lo)] font-[var(--font-title)]">{label}</span>
      <span className="text-[11px] font-semibold font-[var(--font-mono)]" style={{ color: color ?? 'var(--text-hi)' }}>{value}</span>
    </div>
  );
}
...
const inspBase =
  'w-[260px] shrink-0 flex flex-col border-l border-[var(--border)] bg-[var(--panel)] overflow-hidden';
const inspHead =
...
const inspFoot =
  'flex items-center gap-2 px-3 py-2.5 border-t border-[var(--border)] shrink-0';
const footGhost =
