...
export function RewardCard({ reward, selected, onSelect, onDuplicate, onDelete }: RewardCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Status badge */}
      <div className={statBadge} style={{ color: stat.color }}>
      ...
      <div
        className={artWrap}
        style={{ background: `radial-gradient(circle, ${rar.glow} 0%, transparent 70%)` }}
      >
      ...
      <div className={bodyWrap}>
        ...
        {reward.description && <div className={rewardDesc}>{reward.description}</div>}
        ...
        <div className={footRow}>
          ...
          <div className={metaRow}>
            ...
            {reward.stock != null && <span className={metaChip}>{reward.stock} left</span>}
          </div>
        </div>
      </div>
...
const cardSelected = 'border-[var(--rar-color)]';

const cornerBase = 'absolute w-2.5 h-2.5 pointer-events-none border-[1.5px]';
...
const artWrap = 'flex items-center justify-center h-[72px] shrink-0 mt-5';
const artGlyph = 'text-[36px] leading-none drop-shadow-[0_0_12px_var(--rar-glow)] filter';

const bodyWrap = 'flex-1 flex flex-col gap-1 px-2.5 pb-2';
const rewardName =
  'text-[12px] font-bold text-[var(--text-hi)] font-[var(--font-title)] tracking-[0.03em] truncate';
...
const priceGems =
  'flex items-center gap-1 text-[11px] font-bold text-[var(--violet)] font-[var(--font-title)]';
const priceCoins =
  'flex items-center gap-1 text-[11px] font-bold text-[var(--gold)] font-[var(--font-title)]';
...
const metaChip =
  'text-[9px] font-[var(--font-mono)] text-[var(--text-lo)] bg-[var(--panel)] border border-[var(--border)] rounded px-1 py-0.5';

const actionsRow =
  'absolute bottom-0 left-0 right-0 flex items-center justify-center gap-1.5 px-2 py-1.5 bg-[var(--panel2)/90] backdrop-blur-sm opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity duration-150 border-t border-[var(--border)]';
const actionBtn =
