...
const SLOT_PRESETS: SlotPreset[] = [
  ...
  { slot: 'afternoon', glyph: '☉', label: 'Afternoon', time: '14:00', color: 'oklch(0.76 0.16 205)' },
...
export function RescheduleHabitModal({
  ...
  // Default selected slot: pick the one whose preset time differs from current
  const defaultPreset =
    SLOT_PRESETS.find((p) => p.time !== currentTime) ?? SLOT_PRESETS[2]!;
  ...
  const habitName = task.title.replace(/^\S+\s/, ''); // strip leading icon glyph
      ...
      <div className="fixed inset-0 z-[101] flex items-center justify-center p-4 pointer-events-none">
        <div className="pointer-events-auto w-full max-w-[380px] bg-[var(--panel)] border border-[var(--border)] rounded-[var(--r)] shadow-[0_24px_64px_oklch(0_0_0_/_0.6)] flex flex-col">
          ...
          <div className="flex items-center gap-2 px-4 py-3 border-b border-[var(--border)]">
            ...
            <button
              type="button"
              className="w-6 h-6 flex items-center justify-center text-[var(--text-lo)] hover:text-[var(--text-hi)] rounded hover:bg-[var(--panel2)] transition-colors text-[11px]"
              onClick={onClose}
            >
              ✕
            </button>
          ...
          <div className="flex flex-col gap-4 p-4">
            ...
            <div className="flex items-center gap-2 p-2.5 bg-[var(--panel2)] border border-[var(--border)] rounded-[var(--r-sm)]">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[11px] font-bold text-[var(--text-hi)] truncate">{habitName}</div>
                ...
                {SLOT_PRESETS.map((preset) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      disabled={isCurrent}
                      onClick={() => { setSelectedTime(preset.time); setUseCustom(false); }}
                      className={cn(
                    ...
                    >
                      <span className="text-[14px] leading-none shrink-0" style={{ color: preset.color }}>
                        {preset.glyph}
                      </span>
                      ...
                      {isCurrent && (
                        <span className="ml-auto text-[7px] text-[var(--text-dim)] shrink-0">current</span>
                      )}
                    </button>
                  );
                })}
            ...
            {activeTime && (
              <div className="flex items-center gap-2 px-3 py-2 bg-[oklch(0.74_0.17_85_/_0.06)] border border-[oklch(0.74_0.17_85_/_0.2)] rounded-[var(--r-sm)]">
                ...
                <span className="text-[9px] text-[var(--text-mid)]">
                  <span className="font-bold text-[var(--text-hi)]">{habitName}</span>
                  {' '}will move to{' '}
