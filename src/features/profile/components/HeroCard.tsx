  ...
  return (
    <div style={{ width: size, height: size, flexShrink: 0 }}>
      <svg viewBox="-50 -50 100 100" width={size} height={size}>
        ...
        <g style={{ transformOrigin: 'center', transformBox: 'fill-box', animation: 'spin 30s linear infinite' }}>
          <circle r="46" fill="none" stroke={accent.glow} strokeWidth="0.6" strokeDasharray="1 4" opacity="0.7" />
        </g>
...
function BadgeTile({ badge, mini }: BadgeTileProps) {
  ...
  return (
    ...
    >
      <div className="[font-family:var(--f-title)] text-[22px] leading-none italic" style={{ color: accent.glow }}>
        {badge.glyph}
      </div>
      {!mini && <div className="[font-family:var(--f-title)] text-[11px] italic text-text-hi mt-[2px]">{badge.name}</div>}
      {!mini && <div className="[font-family:var(--f-mono)] text-[7px] tracking-[0.18em] text-text-lo uppercase">{badge.era}</div>}
      {!mini && (
        <div className="[font-family:var(--f-mono)] text-[7px] tracking-[0.22em] uppercase" style={{ color: tileColor }}>
          {badge.rarity}
        </div>
      )}
    </div>
...
export function HeroCard({ form }: HeroCardProps) {
  ...
  const showcase = form.badges.map(findBadge).filter(Boolean).slice(0, 4) as NonNullable<ReturnType<typeof findBadge>>[];
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: `radial-gradient(circle at 50% 0%, ${accent.glow}1a, transparent 50%)` }}
      />

      {/* head */}
      <div className="flex justify-between items-start gap-3 relative">
        <div className="min-w-0">
          <div className="[font-family:var(--f-mono)] text-[9px] tracking-[0.28em] uppercase" style={{ color: accent.glow }}>
      ...
      <div
        ...
        style={{ background: 'oklch(18% 0.03 270/0.6)', borderLeft: `2px solid ${accent.glow}`, borderRadius: '0 3px 3px 0' }}
      >
        <span className="text-[18px] leading-none align-[-6px] mr-1" style={{ color: accent.glow, fontFamily: 'var(--f-title)' }}>"</span>
        {form.motto || 'A hero without a motto is a candle without a wick.'}
        <span className="text-[18px] leading-none align-[-6px] ml-1" style={{ color: accent.glow, fontFamily: 'var(--f-title)' }}>"</span>
      </div>

      {/* grid stats */}
      <div className="grid grid-cols-2 border border-border-lo" style={{ background: 'var(--border-lo)', gap: '1px' }}>
        {[
          ...
          <div key={i} className="bg-bg-1 p-[10px]">
            <div className="[font-family:var(--f-mono)] text-[8px] tracking-[0.2em] uppercase text-text-lo">{label}</div>
            {label === 'LEVEL' ? (
              <>
                <div className="[font-family:var(--f-title)] italic text-[22px] leading-none mt-1" style={{ color: accent.glow }}>
                  {value}
                </div>
                <div className="mt-[6px]">
                  <div className="h-[3px] bg-bg-3">
                    <div className="h-full" style={{ width: `${xpPct}%`, background: accent.glow }} />
                  </div>
              ...
              <>
                ...
                {sub && <div className="[font-family:var(--f-mono)] text-[8px] tracking-[0.14em] text-text-lo mt-1">{sub}</div>}
              </>
            )}
          </div>
        ))}
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="[font-family:var(--f-title)] italic text-[14px] text-text-hi">{cls.tagline}</div>
        <div className="[font-family:var(--f-title)] text-[11px] text-text-lo leading-relaxed">{cls.blurb}</div>
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="[font-family:var(--f-title)] italic text-[14px] text-text-hi">{companion.name}</div>
        <div className="[font-family:var(--f-title)] text-[11px] text-text-lo leading-relaxed">{companion.blurb}</div>
        <div className="[font-family:var(--f-mono)] text-[9px] tracking-[0.1em]" style={{ color: accent.glow }}>
          ⟡ {companion.bonus}
        </div>
      </div>
      ...
      <div className="flex justify-between gap-3 pt-3 border-t border-border-lo mt-auto">
        {[
          ...
          <div key={label}>
            <div className="[font-family:var(--f-mono)] text-[8px] tracking-[0.2em] uppercase text-text-lo">{label}</div>
            <div className="[font-family:var(--f-title)] italic text-[12px] text-text-md mt-[2px]">{value}</div>
          </div>
        ))}
      </div>
