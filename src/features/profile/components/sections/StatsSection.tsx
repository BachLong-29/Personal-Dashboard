  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center mb-4 pb-3 border-b border-border-lo">
        <div>
          <div className="[font-family:var(--f-mono)] text-[9px] tracking-[0.22em] uppercase text-text-lo">POOL</div>
        ...
        <div className={remainingCls}>
          {remaining > 0 ? `${remaining} unspent` : remaining === 0 ? 'Allocated' : `${-remaining} over`}
        </div>
      </div>
        ...
        {STAT_KEYS.map(({ key, label, desc }) => (
          <div key={key}>
            <div className="flex justify-between items-baseline mb-[6px]">
              ...
              <span className="[font-family:var(--f-title)] italic text-[20px] text-gold">{stats[key]}</span>
            </div>
            <Slider
              min={1}
              max={10}
