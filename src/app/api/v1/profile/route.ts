...
export const PUT = asyncHandler(async (req: NextRequest) => {
  ...
  const profileKeys = [
    'heroName','title','handle','pronouns','region','sigil',
    'motto','classId','companionId','accent','stats','statPool',
    'primaryFocus','badges',
    'level','xp','xpNext','streak','coins','gems','rank',
  ] as const;

  const settingKeys = [
    'morningRitual','nightlyReview','streakProtection','questDifficulty',
    'seasonalRites','autoReclaim','language','timezone','theme','compactMode',
  ] as const;
